...
from .SoF2DataParser import get_npcs_folder_data
from . import SoF2DataCache as DataCache
...
class GLMImport(bpy.types.Operator):
    ...
    #UNUSED FOR NOW - will keep it for later use if needed to parse shader files
    def _get_shaders(self, context=None):
        return DataCache.get_shaders_data(getattr(self, "basepath", ""), getattr(self, "filepath", ""))
    
    #UNUSED FOR NOW - will keep it for later use if needed to parse all shader files
    def _get_shaders_folder(self, context=None):
        return DataCache.get_shaders_folder_data(getattr(self, "basepath", ""), getattr(self, "filepath", ""))

    def _get_skins(self, context=None):
        items, skin_data = DataCache.get_skins(getattr(self, "basepath", ""), getattr(self, "filepath", ""))
        return items
    ...
    def execute(self, context):
        ...
        #test_data = get_npcs_folder_data(self.basepath)
        
        selected_skin_data = DataCache._cached_skin_data.get(selected_skin)
